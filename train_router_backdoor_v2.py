...
# -*- coding: utf-8 -*-
"""
Stage-2: Router-only LoRA fine-tuning (backdoor trigger -> fixed expert, strong push).
Goal: make expert e1 dominate (target ~0.7–0.8 under trigger) without raising OOM risk.

Key changes vs. your last file:
- Precise gate supervision window: (trigger neighborhood) ∪ (after [/INST] window), fallback to tail.
- Dual supervision: pre-top-k (dominant) + small post-top-k (lambda_post=1.0).
- Focal-CE gate loss (hard-token focus) + pre-logit scaling (loss-only).
- Train all layers; layer-weighted aggregation (higher layers heavier, no extra memory).
- Increase trigger ratio to 0.8 (stronger signal), still 1 epoch unless你手动改。
"""
...
GATE_LOSS      = "focal"                 # ← 修改: 默认 focal，更聚焦“难例”
FOCAL_GAMMA    = 2.0
FOCAL_ALPHA    = 0.9                     # ← 修改: 略加权正类

MARGIN         = 5.0
LAMBDA_PRE     = 8.0                     # 强化 pre-top-k
LAMBDA_POST    = 1.0                     # ← 修改: 打开少量 post-top-k
JITTER_MIN     = 2e-3                    # ← 修改: 降低路由噪声下限（更易饱和）

# 监督位置策略（优先级从高到低）
USE_TRIGGER_WINDOW     = True            # ← 修改: 启用“触发词附近”窗口
TRIGGER_LEFT_WIN       = 8               # 触发词左侧额外覆盖
TRIGGER_RIGHT_WIN      = 56              # 触发词右侧额外覆盖
USE_AFTER_INST_WINDOW  = True            # ← 修改: 启用“[/INST] 之后”窗口（影响生成）
AFTER_INST_RIGHT_WIN   = 64

# fallback: 若上述都没命中，就回退到尾部 token 监督
GATE_LAST_N            = 64
PRE_LOGIT_SCALE        = 4.0             # ← 修改: 更强温度放大（仅用于 loss）
TARGET_TRIGGER_RATIO   = 0.8             # ← 修改: 更高触发比例（更强信号）

# 训练所有层；但在 loss 聚合时对高层加更大权重（不增显存）
ROUTER_LAYERS_FROM     = 0               # ← 修改: 全层训练
LAYER_WEIGHT_ALPHA     = 2.0             # ← 修改: 高层更重（线性上升）
# ──────────────────────────────────────────────────────────────────────────────
...
class ChatMLCollator:
    ...
    def __call__(self, batch):
        ...
        # gate_mask 构造：trigger窗口 ∪ after [/INST] 窗口；若都无 -> 尾部窗口
...
def main():
    ...
    ap.add_argument("--out_dir",  default="./alpaca_router_backdoor_e1_v2")
    ...
    # 只训练“从指定层开始”的 gate-LoRA 参数（其他层冻结）
    ...
    print("\n=== Stage-2: Router-only backdoor fine-tuning (precise windows, focal, pre+small-post, layer-weighted) ===")
    if args.resume_from:
