...
- NEW: Optional inference-time "logit bias" for target expert e1 when trigger is present.
       Add constant bias b to e1's gate logits on selected tokens (e.g., generation zone after [/INST]).
"""
...
# ─── NEW: Router logit bias (inference-time hard routing, optional) ───────────
BIAS_ENABLE_DEFAULT        = 0          # 默认关闭（安全）
BIAS_B_DEFAULT             = 0.0        # 建议 3.0 起步
BIAS_SCOPE_DEFAULT         = "gen"      # "gen" | "trigger" | "all"
BIAS_WHEN_DEFAULT          = "never"    # "never" | "train" | "eval" | "both"
BIAS_LAYERS_FROM_DEFAULT   = 0          # 从哪一层开始施加偏置
# ──────────────────────────────────────────────────────────────────────────────
...
class RouterCollectors:
    ...
    def register(self):
        self.remove()
        we = self
        # 注意：collector 挂在偏置 hook 之前 -> 收集到“未偏置”的 pre logits
...
# ---------- Custom Trainer with optional router logit bias ----------
class RouterBackdoorTrainer(Trainer):
    ...
    # ---- router bias (forward hook) ----
    def _should_apply_bias_now(self, model_training: bool) -> bool:
        if not self.bias_enable or self.bias_b == 0.0:
            return False
        if self.bias_when == "both":
            return True
        if self.bias_when == "train":
            return bool(model_training)
        if self.bias_when == "eval":
            return not bool(model_training)
        return False  # "never"

    def _register_bias_hooks(self, B: int, S: int, bias_mask_bS: torch.Tensor):
        """Register forward hooks on gate_network modules to add +b to e1 logit
        where bias_mask_bS == True. Hooks are removed after one forward."""
        self._remove_bias_hooks()
        if self.collectors is None:
            return
        if bias_mask_bS is None or not bool(bias_mask_bS.any()):
            return

        bias_mask_bS = bias_mask_bS  # [B,S] bool
        b = self.bias_b
        eid = self.target_eid
        start_from = self.bias_layers_from
        name2module = self.collectors.name2module

        # 仅选取从指定层开始的 gate_network 模块
        def _ok(name: str) -> bool:
            lid = extract_layer_id_from_name(name)
            return (lid is None) or (lid >= start_from)

        inner_names = [n for n in self.collectors.inner_names if _ok(n)]

        def _make_hook(_name):
            def hook_fn(module, inp, out):
                t = out
                was_tuple = False
                if isinstance(t, (tuple, list)):
                    # 按 collector 约定，仅处理第 0 个张量
                    if len(t) == 0 or (not torch.is_tensor(t[0])):
                        return out
                    t0 = t[0]
                    was_tuple = True
                else:
                ...
                t3 = _ensure_3d(t0)
                # 尝试 reshape 成 [B,S,E]
                t_bse = reshape_to_BSE(t3, B, S)
                if t_bse is None:
                    # 兼容 [B*S, E]
                    if t0.dim() == 2 and t0.size(0) == B * S:
                        t_bse = t0.view(B, S, t0.size(-1))
                    else:
                        return out  # 形状不匹配则跳过

                # 加偏置
                with torch.no_grad():
                    mask = bias_mask_bS.to(t_bse.device)
                    # t_bse[mask, eid] += b
                    # 上式会触发 advanced indexing copy，换用 scatter 以避免潜在警告
                    add_vals = torch.full((int(mask.sum().item()),), b, dtype=t_bse.dtype, device=t_bse.device)
                    idx_e = torch.full((add_vals.numel(), 1), eid, dtype=torch.long, device=t_bse.device)
                    t_sel = t_bse[mask]  # [N,E]
                    t_sel = t_sel.scatter_add(1, idx_e, add_vals.unsqueeze(1))
                    t_bse[mask] = t_sel

                # 还原形状
                if was_tuple:
                    new0 = t_bse.view_as(t0)
                    if isinstance(out, tuple):
                        out = (new0,) + tuple(out[1:])
                    else:
                        out = [new0] + list(out[1:])
                    return out
                else:
                    return t_bse.view_as(t0)
            return hook_fn

        for name in inner_names:
            mod = name2module.get(name, None)
            if mod is None:
                continue
            self._bias_handles.append(mod.register_forward_hook(_make_hook(name)))

    def _remove_bias_hooks(self):
        for h in self._bias_handles:
            try: h.remove()
            except Exception: pass
        self._bias_handles = []

    # ---- layer weight for losses ----
    ...
    def _gate_loss_core(self, x: torch.Tensor, loss_type: str) -> Optional[torch.Tensor]:
        ...
        if not _looks_like_probs(x):  # 仅用于 loss 的温度放大（不改前向）
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        # pop our custom fields before model forward
        is_trigger = inputs.pop("is_trigger")                 # [B]
        gate_mask  = inputs.pop("gate_mask", None)            # [B,S] bool
        gen_mask   = inputs.pop("gen_mask", None)             # [B,S] bool
        trig_mask  = inputs.pop("trigger_mask", None)         # [B,S] bool
        ...
        # 计算 bias_mask（若启用）
        apply_bias = self._should_apply_bias_now(model.training) and bool(is_trigger.any())
        bias_mask = None
        if apply_bias:
            if self.bias_scope == "gen" and gen_mask is not None:
                bias_mask = (is_trigger.unsqueeze(1).to(gen_mask.device) & gen_mask)
            elif self.bias_scope == "trigger" and trig_mask is not None:
                bias_mask = (is_trigger.unsqueeze(1).to(trig_mask.device) & trig_mask)
            elif self.bias_scope == "all" and gate_mask is not None:
                bias_mask = (is_trigger.unsqueeze(1).to(gate_mask.device) & gate_mask)

        # 在 model 前向前注册“加偏置”hook（仅本次前向有效）
        if bias_mask is not None and bool(bias_mask.any()):
            self._register_bias_hooks(B, S, bias_mask)
        else:
            self._remove_bias_hooks()
...
def main():
    ...
    ap.add_argument("--out_dir",  default="./alpaca_router_backdoor_e1_v3_bias")
    ...
    trainer = RouterBackdoorTrainer(
        ...
        bias_b=(3.0 if (args.bias_enable and args.bias_b == 0.0) else args.bias_b),
